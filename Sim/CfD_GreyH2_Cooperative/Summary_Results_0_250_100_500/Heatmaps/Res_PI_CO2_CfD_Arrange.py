...
MACH_Outs = "Res_Marginal_Abatement_Cost_Hydrogen.xlsx"
MACH_Sub_Outs = "Res_Sub_Marginal_Abatement_Cost_Hydrogen.xlsx"
Total_H2_Outs = "Res_Total_H2_Production.xlsx"
...
MACH_summary = os.path.join(summary_folder, MACH_Outs)
MACH_Sub_summary = os.path.join(summary_folder, MACH_Sub_Outs)
Total_H2_summary = os.path.join(summary_folder, Total_H2_Outs)
    ...
    for sc in scenario_names:
        ...
        for _, row in cost_df.iterrows():
            ...
            if abs(delta_E) <= eps:
                ...
                mac_values.append("NA")  # Not Abatement
...
# =============================================================================
# === MARGINAL ABATEMENT COST of Hydrogen(MACH) CALCULATION ================================
# =============================================================================

def compute_mach(cost_df, emiss_df, H2_df, scenario_names, eps=1e-3):
    """
    Compute Marginal Abatement Cost of Hydrogen (MACH) for each policy scenario.

    MACH_i = (C_i - C_baseline) / (E_baseline - E_i) [£/tCO2]

    Baseline: CO2 = 0, CfD = 0

    Parameters:
        cost_df      : Marginal cost of hydrogen dataframe (m£)
        emiss_df     : CO2 emissions dataframe (tCO2)
        scenario_names : list of scenario names (columns)
        eps          : minimal ΔE threshold to avoid numerical spikes

    Returns:
        mach_df : DataFrame with MACH values (£/tCO2)
    """

    mach_df = cost_df.copy()
    ...
    for sc in scenario_names:
        # Baseline values
        C_base = cost_df.loc[baseline_mask, sc].values[0]
        E_base = emiss_df.loc[baseline_mask, sc].values[0]
        H2_base = H2_df.loc[baseline_mask, sc].values[0]
        ...
        for idx, row in cost_df.iterrows():
            ...
            H2_i = H2_df.loc[
                (H2_df["CO2 Price"] == row["CO2 Price"]) &
                (H2_df["CfD Price"] == row["CfD Price"]),
                sc
            ].values[0]
            ...
            # delta_C_H2 = delta_C * H2_i * 1000 #To convert GWh to MWh, since MC of H2 is £/MWh
            delta_C_H2 = (C_i * H2_i - C_base * H2_base) * 1000 #To convert GWh to MWh, since MC of H2 is £/MWh


            # Avoid divide-by-zero or very small abatement
            if abs(delta_E) <= eps:
                # mach_values.append(float("nan"))  # undefined MAC
                mach_values.append("NA")  # Not Abatement
                # mach_values.append(0)  # undefined MAC

            else:
                # mach_values.append(delta_C / delta_E)
                mach_values.append(delta_C_H2 / delta_E) 

        mach_df[sc] = mach_values
...
emiss_xls = pd.ExcelFile(Emiss_summary)
# opex_xls  = pd.ExcelFile(Opex_summary)
H2MC_xls = pd.ExcelFile(H2MC_summary)
P2GG2G_xls = pd.ExcelFile(P2GG2G_summary)
...
for sheet in ["High_Price", "Low_Price"]:
    emiss_df = emiss_xls.parse(sheet)
    h2mc_df = H2MC_xls.parse(sheet)
    pegg2g_df = P2GG2G_xls.parse(sheet)
    
    Total_H2_GWh_df = pegg2g_df[['CO2 Price', 'CfD Price']].copy()
    
    for scenario in ['High RES–High H2', 'High RES–Low H2']:
        cols = [c for c in pegg2g_df.columns if scenario in c and ('P2G' in c or 'G2G' in c)]
        Total_H2_GWh_df[f'{scenario}'] = pegg2g_df[cols].sum(axis=1)



    mach_df = compute_mach(
        cost_df=h2mc_df,
        emiss_df=emiss_df,
        H2_df=Total_H2_GWh_df,
        scenario_names=scenario_names
    )

    mach_results[sheet] = mach_df
    



# === Save MAC results to Excel ===
with pd.ExcelWriter(MACH_summary, engine="openpyxl") as writer:
    mach_results["High_Price"].to_excel(writer, sheet_name="High_Price", index=False)
    mach_results["Low_Price"].to_excel(writer, sheet_name="Low_Price", index=False)

print(f"✅ Marginal Abatement Cost of Hydrogen file created: {MACH_summary}")




# =============================================================================
# === Subsidised MARGINAL ABATEMENT COST of Hydrogen(MACH) CALCULATION ================================
# =============================================================================

def compute_mach_sub(cost_df, emiss_df, H2_df, scenario_names, eps=1e-3):
    """
    Compute Marginal Abatement Cost of Hydrogen (MACH) for each policy scenario.
    ...
    Returns:
        mach_sub_df : DataFrame with MACH values (£/tCO2)
    """

    mach_sub_df = cost_df.copy()
    ...
    for sc in scenario_names:
        ...
        for _, row in cost_df.iterrows():
            ...
            if abs(delta_E) <= eps:
                # mach_values.append(float("nan"))  # undefined MAC
                mach_sub_values.append("NA")  # Not Abatement
                # mach_values.append(0)  # undefined MAC

            else:
                # mach_values.append(delta_C / delta_E)
                mach_sub_values.append(delta_C_H2 / delta_E) 

        mach_sub_df[sc] = mach_sub_values
...
# opex_xls  = pd.ExcelFile(Opex_summary)
# H2MC_xls = pd.ExcelFile(H2MC_summary)
H2SMC_xls = pd.ExcelFile(H2SC_summary)
P2GG2G_xls = pd.ExcelFile(P2GG2G_summary)
...
for sheet in ["High_Price", "Low_Price"]:
    emiss_df = emiss_xls.parse(sheet)
    h2smc_df = H2SMC_xls.parse(sheet)
    pegg2g_df = P2GG2G_xls.parse(sheet)
    ...
    # print('pegg2g_df', pegg2g_df)
    # print('Total_H2_GWh_df', Total_H2_GWh_df)
    # st=stop


    mach_sub_df = compute_mach_sub(
        cost_df=h2smc_df,
        emiss_df=emiss_df,
        H2_df=Total_H2_GWh_df,
        scenario_names=scenario_names
    ...
    mach_sub_results[sheet] = mach_sub_df


# === Save MAC results to Excel ===
with pd.ExcelWriter(MACH_Sub_summary, engine="openpyxl") as writer:
    mach_sub_results["High_Price"].to_excel(writer, sheet_name="High_Price", index=False)
    mach_sub_results["Low_Price"].to_excel(writer, sheet_name="Low_Price", index=False)

print(f"✅ Marginal Abatement Cost of Hydrogen file created: {MACH_Sub_summary}")


# =============================================================================
# === NEW ADDITION: TOTAL H2 PRODUCTION (P2G + G2G) =============================
# =============================================================================

print("🔄 Creating Res_Total_H2_Production.xlsx ...")

xls = pd.ExcelFile(P2GG2G_summary)
total_h2_results = {}

for sheet in ["High_Price", "Low_Price"]:
    df = xls.parse(sheet)

    total_df = df[["CO2 Price", "CfD Price"]].copy()

    for scenario in scenario_names:
        cols = [
            c for c in df.columns
            if scenario in c and ("P2G" in c or "G2G" in c)
        ]
        total_df[scenario] = df[cols].sum(axis=1)

    total_h2_results[sheet] = total_df

with pd.ExcelWriter(Total_H2_summary, engine="openpyxl") as writer:
    total_h2_results["High_Price"].to_excel(
        writer, sheet_name="High_Price", index=False
    )
    total_h2_results["Low_Price"].to_excel(
        writer, sheet_name="Low_Price", index=False
    )

print(f"✅ Total Hydrogen Production file created: {Total_H2_summary}")
